...
class YealinkPlugin(common_globals['BaseYealinkPlugin']):
    ...
    def _format_funckey(self, lines, funckey_no, model, prefix, funckey, exten_pickup_call):
        ...
        if funckey_type == u'speeddial':
            ...
            self._format_funckey_blf(lines, prefix, funckey, exten_pickup_call, model)
    ...
    def _format_funckey_blf(self, lines, prefix, funckey, exten_pickup_call, model):
        line_no = funckey.get(u'line', 1)
        if model in (u'T32G', 'T38G'):
            line_no -= 1
        lines.append(u'%s.type = 16' % prefix)
        lines.append(u'%s.line = %s' % (prefix, line_no))
        lines.append(u'%s.value = %s' % (prefix, funckey[u'value']))
        lines.append(u'%s.label = %s' % (prefix, funckey.get(u'label', u'')))
