...
class TestPluginAssociation:
    def test_plugin_association_when_all_info_match(self, v87_entry):
        plugin_associator = BaseYealinkPgAssociator(v87_entry.MODEL_INFO)
        for model, info in v87_entry.MODEL_INFO.items():
            support = plugin_associator._do_associate('Yealink', model, info['version'])
            assert support == DeviceSupport.EXACT

    def test_plugin_association_when_only_vendor_and_model_match(self, v87_entry):
        plugin_associator = BaseYealinkPgAssociator(v87_entry.MODEL_INFO)
        for model in v87_entry.MODEL_INFO:
            support = plugin_associator._do_associate('Yealink', model, None)
            assert support == DeviceSupport.COMPLETE

    def test_plugin_association_when_only_vendor_matches(self, v87_entry):
        plugin_associator = BaseYealinkPgAssociator(v87_entry.MODEL_INFO)
        support = plugin_associator._do_associate('Yealink', None, None)
        assert support == DeviceSupport.PROBABLE

    def test_plugin_association_when_nothing_matches(self, v87_entry):
        plugin_associator = BaseYealinkPgAssociator(v87_entry.MODEL_INFO)
    ...
    def test_plugin_association_does_not_match_when_empty_strings(self, v87_entry):
        plugin_associator = BaseYealinkPgAssociator(v87_entry.MODEL_INFO)
        assert plugin_associator._do_associate('', '', '') == DeviceSupport.IMPROBABLE


class TestPlugin:
    @patch('plugins.wazo_yealink.v87.common.FetchfwPluginHelper')
    def test_init(self, fetch_fw, v87_entry):
        ...
        plugin = v87_entry.YealinkPlugin(
    ...
    def test_configure(self, v87_plugin):
        ...
        v87_plugin._tpl_helper.get_dev_template.return_value = 'template'
        v87_plugin.configure(device, raw_config)
        v87_plugin._tpl_helper.get_dev_template.assert_called_with(
            '805ec0d57d72.cfg', device
        )
        v87_plugin._tpl_helper.dump.assert_called_with(
            'template', raw_config, 'test_dir/var/tftpboot/805ec0d57d72.cfg', 'UTF-8'
        )

    @patch('os.remove')
    def test_deconfigure(self, mocked_remove, v87_plugin):
        v87_plugin.deconfigure({'mac': '80:5e:c0:d5:7d:72'})
        mocked_remove.assert_called_with('test_dir/var/tftpboot/805ec0d57d72.cfg')

    @patch('plugins.wazo_yealink.v87.common.logger')
    def test_deconfigure_no_file(self, mocked_logger, v87_plugin):
        v87_plugin.deconfigure({'mac': '00:00:00:00:00:00'})
    ...
    @patch('plugins.wazo_yealink.v87.common.synchronize')
    def test_synchronize(self, provd_synchronize, v87_plugin):
        device = {'mac': '80:5e:c0:d5:7d:72'}
        v87_plugin.synchronize(device, {})
        provd_synchronize.standard_sip_synchronize.assert_called_with(device)

    def test_get_remote_state_trigger_filename(self, v87_plugin):
        assert v87_plugin.get_remote_state_trigger_filename({}) is None
        assert (
            v87_plugin.get_remote_state_trigger_filename({'mac': '80:5e:c0:d5:7d:72'})
            == '805ec0d57d72.cfg'
        )

    def test_sensitive_file(self, v87_plugin):
        assert v87_plugin.is_sensitive_filename('patate') is False
        assert v87_plugin.is_sensitive_filename('805ec0d57d72.cfg') is True

    def test_add_country_and_lang(self, v87_plugin):
        raw_config = {'locale': 'en_US'}
        v87_plugin._add_country_and_lang(raw_config)
    ...
    def test_checks(self, v87_plugin):
        with pytest.raises(RawConfigError):
            v87_plugin._check_config({})

        with pytest.raises(Exception):
            v87_plugin._check_device({})

    @patch('plugins.wazo_yealink.v87.common.plugins')
    def test_phonebook_url(self, provd_plugins, v87_plugin):
        raw_config = {'config_version': 1}
        v87_plugin._add_xivo_phonebook_url(raw_config)
    ...
    def test_sip_lines(self, v87_plugin):
        raw_config = {
        ...
        v87_plugin._add_xx_sip_lines({'model': 'patate'}, raw_config)
        assert raw_config['sip_lines'] == raw_config['XX_sip_lines']

        v87_plugin._update_sip_lines(raw_config)
        v87_plugin._add_xx_sip_lines({'model': 'T33G'}, raw_config)
    ...
    @patch('plugins.wazo_yealink.v87.common.logger')
    def test_timezones(self, mocked_logger, v87_plugin):
        raw_config = {'timezone': 'America/Montreal'}
        v87_plugin._add_timezone(raw_config)
        ...
        v87_plugin._add_timezone({'timezone': 'Doesnt/Exist'})
    ...
    @patch('plugins.wazo_yealink.v87.common.logger')
    def test_function_keys(self, mocked_logger, v87_plugin):
        raw_config = {
            'funckeys': {
                '1': {
                    'type': 'speeddial',
    ...
    def test_fkeys_with_exp40(self, v87_plugin):
        base_raw_config = {
            'funckeys': {
                '1': {
                    'type': 'speeddial',
