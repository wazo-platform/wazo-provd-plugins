...
def build_10_1_49_11(path):
    ...
    for model, fw_suffix in MODELS:
        # generate snom<model>-firmware.xml.tpl from snom-model-firmware.xml.tpl.btpl
        model_tpl = template_dir / f'snom{model}-firmware.xml.tpl'
        sed_script = f's/#FW_FILENAME#/snom{model}-10.1.49.11-SIP-{fw_suffix}.bin/'
        with model_tpl.open(mode='wb') as f:
            check_call(
                [
                    'sed',
...
@target('10.1.198.19', 'wazo-snom-10.1.198.19')
def build_10_1_198_19(path):
    MODELS = [
        ('D140', False),
        ('D150', False),
        ('D315', True),
    ...
    for model, ramdisk in MODELS:
        # generate snom<model>-firmware.xml.tpl from snom-model-firmware.xml.tpl.btpl
        model_tpl = template_dir / f'snom{model}-firmware.xml.tpl'
        sed_script = f's/#FW_FILENAME#/snom{model}-10.1.198.19-SIP-r.bin/'
        sed_script += ';s/&chained_url=.*{mac}//'
        if model.startswith("D8"):
            sed_script = f's/#FW_FILENAME#/snom{model}-10.1.198.19-SIP-r.swu/'
            sed_script += ';s/&chained_url=.*{mac}//'
        if ramdisk:
            sed_script = f's/#FW_FILENAME#/snom{model}-ramdisk-chained-update-r.bin/'
...
@target('10.1.226.16', 'wazo-snom-10.1.226.16')
def build_10_1_226_16(path):
    MODELS = [
        ('D140', False),
        ('D150', False),
        ('D335', True),
    ...
    for model, ramdisk in MODELS:
        # generate snom<model>-firmware.xml.tpl from snom-model-firmware.xml.tpl.btpl
        model_tpl = template_dir / f'snom{model}-firmware.xml.tpl'
        sed_script = f's/#FW_FILENAME#/snom{model}-10.1.226.16-SIP-r.bin/'
        sed_script += ';s/&chained_url=.*{mac}//'
        if model.startswith("D8"):
            sed_script = f's/#FW_FILENAME#/snom{model}-10.1.226.16-SIP-r.swu/'
            sed_script += ';s/&chained_url=.*{mac}//'
        if ramdisk:
            sed_script = f's/#FW_FILENAME#/snom{model}-ramdisk-chained-update-r.bin/'
            sed_script += f';s/#CHAINED_FILENAME#/snom{model}-10.1.226.16-SIP-r.bin/'
        if model.startswith("D8"):
            sed_script += ';s/#UXMC_FILENAME#/snomD8C-1.9.1-r.bin/'
        elif model in ['D717', 'D735', 'D785']:
            sed_script += ';s/#UXMC_FILENAME#/snomD7C-1.9.1-r.bin/'
        else:
            sed_script += ';/<firmware_uxm>/d'
    ...
    check_call(['rsync', '-rlp', '--exclude', '.*', 'v10_1_226_16/', path])
