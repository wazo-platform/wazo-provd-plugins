...
-->
    ...
    {%- if http_port %}
    <S_SPECIAL_DATA_SRV class="string" value='"{{ XX_server_url }}/42/fw262.bin?mac={{ XX_mac_addr }}&fw262.bin"'/>
    {%- endif %}
...
<!-- WEB UI: Settings - Management - Firmware update - Data Server
    ...
    {%- if http_port %}
    <SYMB_ITEM ID="BS_IP_Data1.aucS_DATA_SERVER[0]" class="symb_item" value='"{{ XX_server_url_without_scheme }}"'/>
    <SYMB_ITEM ID="BS_IP_Data1.aucS_USER_FIRMWARE_URL" class="string" value='"{{ XX_server_url_without_scheme }}/42/fw262.bin"' />
    <S_CONFIGFILE_URL class="string" value='"{{ XX_server_url_without_scheme }}/{{ XX_mac_addr }}.xml"'/>
    {%- endif %}
