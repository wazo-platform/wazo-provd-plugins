...
logger = logging.getLogger('plugin.xivo-mitel')


class BaseMitelHTTPDeviceInfoExtractor(object):
    _UA_REGEX = re.compile(r'^Mitel(\w+) MAC:([^ ]+) V:([^ ]+)-SIP$')
...
class BaseMitelPgAssociator(BasePgAssociator):
...
class BaseMitelPlugin(StandardPlugin):
    _ENCODING = 'UTF-8'
    _KEYTYPE = {
        # <model>: ([(<nb keys>, <keytype>), ...], <nb expansion modules>)
        u'6863i': ([(3, u'prgkey')], 0),
        u'6865i': ([(8, u'prgkey')], 3),
        u'6867i': ([(6, u'prgkey'), (20, u'softkey')], 3),
        u'6869i': ([(12, u'prgkey'), (40, u'softkey')], 3),
    }
    ...
    def __init__(self, app, plugin_dir, gen_cfg, spec_cfg):
        ...
        self._tftpboot_dir = os.path.join(self._tftpboot_dir, 'Mitel')
    ...
    http_dev_info_extractor = BaseMitelHTTPDeviceInfoExtractor()
