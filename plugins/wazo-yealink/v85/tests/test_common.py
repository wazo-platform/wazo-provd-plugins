...
class TestPluginAssociation(object):
    def test_plugin_association_when_all_info_match(self, v85_entry):
        ...
        for model, version in six.iteritems(v85_entry.MODEL_VERSIONS):
            assert_that(
    ...
    def test_plugin_association_when_only_vendor_and_model_match(self, v85_entry):
        plugin_associator = BaseYealinkPgAssociator(v85_entry.MODEL_VERSIONS)
        for model in six.iterkeys(v85_entry.MODEL_VERSIONS):
            assert_that(
...
class TestPlugin(object):
    @patch('v85.common.FetchfwPluginHelper')
    def test_init(self, fetch_fw, v85_entry):
        ...
        plugin = v85_entry.YealinkPlugin(MagicMock(), 'test_dir', MagicMock(), MagicMock())
        assert_that(
            plugin,
            has_properties(
    ...
    def test_configure(self, v85_plugin):
        ...
        v85_plugin._tpl_helper.get_dev_template.assert_called_with('805ec0d57d72.cfg', device)
        v85_plugin._tpl_helper.dump.assert_called_with(
            'template', raw_config, 'test_dir/var/tftpboot/805ec0d57d72.cfg', 'UTF-8'
        )

    @patch('os.remove')
    def test_deconfigure(self, mocked_remove, v85_plugin):
        v85_plugin.deconfigure({'mac': '80:5e:c0:d5:7d:72'})
        mocked_remove.assert_called_with('test_dir/var/tftpboot/805ec0d57d72.cfg')

    @patch('v85.common.logger')
    def test_deconfigure_no_file(self, mocked_logger, v85_plugin):
        v85_plugin.deconfigure({'mac': '00:00:00:00:00:00'})
    ...
    @patch('v85.common.synchronize')
    def test_synchronize(self, provd_synchronize, v85_plugin):
        device = {'mac': '80:5e:c0:d5:7d:72'}
        v85_plugin.synchronize(device, {})
        provd_synchronize.standard_sip_synchronize.assert_called_with(device)

    def test_get_remote_state_trigger_filename(self, v85_plugin):
        assert v85_plugin.get_remote_state_trigger_filename({}) is None
        assert v85_plugin.get_remote_state_trigger_filename(
            {'mac': '80:5e:c0:d5:7d:72'}
        ) == '805ec0d57d72.cfg'

    def test_sensitive_file(self, v85_plugin):
        assert v85_plugin.is_sensitive_filename('patate') is False
        assert v85_plugin.is_sensitive_filename('805ec0d57d72.cfg') is True

    def test_add_country_and_lang(self, v85_plugin):
        raw_config = {'locale': 'en_US'}
        v85_plugin._add_country_and_lang(raw_config)
    ...
    def test_checks(self, v85_plugin):
        with pytest.raises(RawConfigError):
            v85_plugin._check_config({})

        with pytest.raises(Exception):
            v85_plugin._check_device({})

    @patch('v85.common.plugins')
    def test_phonebook_url(self, provd_plugins, v85_plugin):
        raw_config = {'config_version': 1}
        v85_plugin._add_xivo_phonebook_url(raw_config)
        provd_plugins.add_xivo_phonebook_url.assert_called_with(
            raw_config, 'yealink', entry_point='lookup', qs_suffix='term=#SEARCH',
        )

    def test_sip_lines(self, v85_plugin):
        raw_config = {
        ...
        v85_plugin._add_xx_sip_lines({'model': 'patate'}, raw_config)
        assert raw_config['sip_lines'] == raw_config['XX_sip_lines']

        v85_plugin._update_sip_lines(raw_config)
        v85_plugin._add_xx_sip_lines({'model': 'T33G'}, raw_config)
    ...
    @patch('v85.common.logger')
    def test_timezones(self, mocked_logger, v85_plugin):
    ...
    @patch('v85.common.logger')
    def test_function_keys(self, mocked_logger, v85_plugin):
        raw_config = {
            'funckeys': {
                '1': {
                    'type': 'speeddial',
        ...
        v85_plugin._add_fkeys({'model': 'T32G'}, raw_config)
    ...
    def test_fkeys_with_exp40(self, v85_plugin):
        base_raw_config = {
            'funckeys': {
                '1': {
                    'type': 'speeddial',
        ...
        v85_plugin._add_fkeys({'model': 'T27G'}, raw_config)
        assert raw_config['XX_fkeys'] == TEST_LINES + self._build_exp_expectation(4, 21, 40)
        raw_config = dict(**base_raw_config)
        v85_plugin._add_fkeys({'model': 'T5'}, raw_config)
